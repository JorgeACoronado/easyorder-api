...
const adminMenuItems = new Hono()

adminMenuItems.get('/', async (c) => {
...
adminMenuItems.post('/', async (c) => {
  ...
  c.header('Location', `/api/admin/menu-items/${menuItem.id}`)

  return sendResource(c, menuItem, 201)
})

adminMenuItems.get('/:id', async (c) => {
  const userId = c.get('user').sub
...
adminMenuItems.patch('/:id', async (c) => {
  const userId = c.get('user').sub
  const id = parseIdParam(c.req.param('id'))
  const payload = await parseJsonBody(c)
...
adminMenuItems.delete('/:id', async (c) => {
  const userId = c.get('user').sub
  const id = parseIdParam(c.req.param('id'))
